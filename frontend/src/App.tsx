...
export default function App() {
  ...
  async function createAndRun() {
    ...
    setBusy(true)
    try {
      const create = await fetch('/api/projects', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ name:projectName(prompt), description:prompt, stack:'React + Spring Boot + PostgreSQL' }) })
      ...
      await execute(project, mode, prompt)
  ...
  async function followUp() {
    ...
    setBusy(true)
    try { await execute(selected, mode, prompt) }
    catch (error) { setActivity([error instanceof Error ? error.message : 'Command failed']) }
    finally { setBusy(false) }
  }

  function openProject(project: Project) { setSelected(project); setView('builder'); setPrompt(project.description || '') }
  ...
  if (view === 'builder' && selected) return <Builder project={selected} prompt={prompt} setPrompt={setPrompt} mode={mode} setMode={setMode} busy={busy} activity={activity} run={followUp} back={()=>setView('dashboard')} />
  ...
  return <div className="creator-shell">
    ...
    <main className="creator-main">
      ...
      <section className="projects-area"><div className="projects-head"><div><h2>Your projects</h2><p>Continue building or start from a fresh idea.</p></div><button onClick={()=>{setPrompt('');window.scrollTo({top:0,behavior:'smooth'})}}>＋ New project</button></div>{projects.length===0?<div className="empty-projects"><b>No projects yet</b><span>Your first generated application will appear here.</span></div>:<div className="project-grid">{projects.map(project=><button className="project-card" key={project.id} onClick={()=>openProject(project)}><div className="project-thumb"><span>✦</span><div><i></i><i></i><i></i></div></div><div className="project-info"><b>{project.name}</b><span>{project.description || project.stack}</span><small>{project.status} · {project.stack}</small></div></button>)}</div>}</section>
    </main>
    {searchOpen&&<div className="search-backdrop" onMouseDown={()=>setSearchOpen(false)}><div className="command" onMouseDown={e=>e.stopPropagation()}><input autoFocus value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search projects and commands…"/><div className="command-label">Projects</div>{results.length===0?<div className="command-empty">No matching projects</div>:results.map(project=><button key={project.id} onClick={()=>{openProject(project);setSearchOpen(false)}}><span>▱</span><div><b>{project.name}</b><small>{project.description}</small></div></button>)}</div></div>}
  </div>
}
...
function Builder({project,prompt,setPrompt,mode,setMode,busy,activity,run,back}:{project:Project;prompt:string;setPrompt:(v:string)=>void;mode:'build'|'plan';setMode:(v:'build'|'plan')=>void;busy:boolean;activity:string[];run:()=>void;back:()=>void}) {
  return <div className="builder-shell"><header className="builder-top"><button onClick={back}>← Dashboard</button><b>{project.name}</b><div><button>GitHub</button><button>Share</button><button className="publish">Publish</button></div></header><aside className="builder-chat"><div className="chat-title"><b>ForgePilot</b><span>{mode==='build'?'Agent':'Plan'}</span></div><div className="chat-body"><div className="user-prompt">{project.description}</div>{activity.map((line,i)=><div className="agent-message" key={`${i}-${line}`}>{line}</div>)}</div><div className="builder-composer"><textarea value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder="Ask ForgePilot to make a change…"/><div><button onClick={()=>setMode(mode==='build'?'plan':'build')}>{mode==='build'?'Build':'Plan'}⌄</button><button className="send" disabled={busy||!prompt.trim()} onClick={run}>{busy?'…':'↑'}</button></div></div></aside><main className="live-preview"><div className="preview-top"><div><button className="active">Preview</button><button>Code</button></div><span>Desktop ▾</span><div><button>↻</button><button>↗</button></div></div><div className="preview-stage"><div className="preview-browser"><div className="fake-app"><aside><b>Acme</b><span className="active">Dashboard</span><span>Customers</span><span>Analytics</span><span>Settings</span></aside><main><small>GENERATED BY FORGEPILOT</small><h1>{project.name}</h1><p>{project.description}</p><div className="fake-stats"><div><b>1,284</b><span>Customers</span></div><div><b>87%</b><span>Completion</span></div><div><b>24</b><span>Active flows</span></div></div><section><b>Your application is ready to iterate</b><p>Continue prompting on the left. Live executable preview is the next product milestone.</p></section></main></div></div></div></main></div>
}
