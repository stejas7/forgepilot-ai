...
export default function VisualPreview({projectId,projectName,refresh,setRefresh,viewport,onTargetPrompt}:Props){
  ...
  function handlePreviewClick(event:Event){if(!inspectMode)return;event.preventDefault();event.stopPropagation();const target=event.target;if(!(target instanceof Element))return;const ownerWindow=target.ownerDocument.defaultView;if(!ownerWindow)return;const FrameHTMLElement=(ownerWindow as Window & typeof globalThis).HTMLElement;if(!(target instanceof FrameHTMLElement))return;selectElement(target as HTMLElement)}
